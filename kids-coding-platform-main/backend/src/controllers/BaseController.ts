...
export abstract class BaseController {
    ...
    protected validateCoppaCompliance(
        ...
        const coppa = user.coppa as any;

        if (coppa?.requiresParentalConsent && !coppa?.parentalConsent) {
    ...
    protected checkUserSafety(
        ...
        const user = req.user;
        const coppa = user.coppa as any;
        const safetySettings = user.safety as any;

        // Check time limits for younger users
        if (coppa?.requiresParentalConsent && safetySettings?.parentalControls) {
